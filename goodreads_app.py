...
'''
Hey there!  Welcome to Tyler's Goodreads Analysis App. The app
analyzes (and never stores!) the books you've read using the 
popular services Goodreads, including looking at the
distribution of the age and lengthe of books you've read.  
Give  it a go by uploading you data below!
'''
...
# filtering to include positive numbder of days an to only finished books
books_df['days_to_finish'] = (pd.to_datetime(books_df['Date Read']) - 
                             pd.to_datetime(books_df['Date Added'])).dt.days

books_finished_filtered = books_df[(books_df['Exclusive Shelf'] ==
                                   'read') & (books_df['days_to_finish'] >= 0)]
...
# calculate whether we rate books higher or lower thatn the Goodreads average
import numpy as np
avg_difference = np.round(np.mean(books_rated['My Rating'] -
                                 books_rated['Average Rating']), 2)
if avg_difference >= 0:
    sign = 'higher'
else:
    sign = 'lower'
...
st.write(f'It looks like you have finished {u_books} books \
        with a total of {u_authors} unique authors.  You most \
        read author is {mode_author}!')
st.write('Your app results can be found below, we have \
        analyzed everything from your book length distribution \
        to how you rate books.  Take a look around, all the graphs \
        are interactive!')
...
with row1_col2:
    ...
    st.write(f'It took you an average of {mean_days_to_finish} \
            days between when the book was added to Goodreads and \
            when you finished the book.  This is not a perfect metric, \
            as you may have added this book to a to-read list!')
with row2_col1:
    st.plotly_chart(fig_num_pages)
    avg_pages = int(books_df['Number of Pages'].mean())
    st.write(f'Your books are an average of {avg_pages} pages \
            long, check out the distribution above!')
with row2_col2:
    st.plotly_chart(fig_year_published)
    st.write('This chart is zoomed into the period of 1850-2021, \
            but is interactive so try zooming in/out on \
            interesting periods!')
with row3_col1:
    st.plotly_chart(fig_my_rating)
    avg_my_rating = round(books_rated['My Rating'].mean(), 2)
    st.write(f'You rate books an average of {avg_my_rating} \
            stars on Goodreads')
with row3_col2:
    st.plotly_chart(fig_avg_rating)
    st.write(f'You rate books {sign} than the average \
        Goodreads user by {abs(avg_difference)}!')
